...
abstract class AbstractObjectNormalizer extends AbstractNormalizer
{
    ...
    public function __construct(
        ...
        if (isset($this->defaultContext[self::MAX_DEPTH_HANDLER]) && !\is_callable($this->defaultContext[self::MAX_DEPTH_HANDLER])) {
            throw new InvalidArgumentException(sprintf('The "%s" given in the default context is not callable.', self::MAX_DEPTH_HANDLER), 2524186855);
        }
    ...
    public function normalize(mixed $object, ?string $format = null, array $context = [])
    {
        ...
        if (isset($context[self::MAX_DEPTH_HANDLER])) {
            ...
            if (!\is_callable($maxDepthHandler)) {
                throw new InvalidArgumentException(sprintf('The "%s" given in the context is not callable.', self::MAX_DEPTH_HANDLER), 3093413528);
            }
        } else {
        ...
        foreach ($stack as $attribute => $attributeValue) {
            ...
            if (!$this->serializer instanceof NormalizerInterface) {
                throw new LogicException(sprintf('Cannot normalize attribute "%s" because the injected serializer is not a normalizer.', $attribute), 3185877987);
            }
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = [])
    {
        ...
        foreach ($normalizedData as $attribute => $value) {
            if ($this->nameConverter) {
                ...
                if (isset($nestedData[$notConverted]) && !isset($originalNestedData[$attribute])) {
                    throw new LogicException(sprintf('Duplicate values for key "%s" found. One value is set via the SerializedPath attribute: "%s", the other one is set via the SerializedName attribute: "%s".', $notConverted, implode('->', $nestedAttributes[$notConverted]->getElements()), $attribute), 3632415765);
                }
            }
        ...
        if ($extraAttributes) {
            throw new ExtraAttributesException($extraAttributes, 2661625555);
        }
            ...
            try {
                ...
                if (Type::BUILTIN_TYPE_OBJECT === $builtinType && null !== $class) {
                    if (!$this->serializer instanceof DenormalizerInterface) {
                        throw new LogicException(sprintf('Cannot denormalize attribute "%s" for class "%s" because injected serializer is not a denormalizer.', $attribute, $class), 4207315712);
                    }
    ...
    private function updateData(array $data, string $attribute, mixed $attributeValue, string $class, ?string $format, array $context, ?array $attributesMetadata, ?ClassMetadataInterface $classMetadata): array
    {
        ...
        if (null !== $classMetadata && null !== $serializedPath = ($attributesMetadata[$attribute] ?? null)?->getSerializedPath()) {
            ...
            if ($propertyAccessor->isReadable($data, $serializedPath) && null !== $propertyAccessor->getValue($data, $serializedPath)) {
                throw new LogicException(sprintf('The element you are trying to set is already populated: "%s".', (string) $serializedPath), 5420353685);
            }
    ...
    private function getNestedAttributes(string $class): array
    {
        ...
        foreach ($classMetadata->getAttributesMetadata() as $name => $metadata) {
            ...
            if (isset($serializedPaths[$pathIdentifier])) {
                throw new LogicException(sprintf('Duplicate serialized path: "%s" used for properties "%s" and "%s".', $pathIdentifier, $serializedPaths[$pathIdentifier], $name), 5561742219);
            }
