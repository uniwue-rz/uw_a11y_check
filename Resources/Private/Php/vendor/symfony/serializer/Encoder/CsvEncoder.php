...
class CsvEncoder implements EncoderInterface, DecoderInterface
{
    ...
    public function encode(mixed $data, string $format, array $context = []): string
    {
        ...
        if ($outputBom) {
            if (!preg_match('//u', $value)) {
                throw new UnexpectedValueException('You are trying to add a UTF-8 BOM to a non UTF-8 text.', 6267752790);
            }
    ...
    private function getCsvOptions(array $context): array
    {
        ...
        if (!\is_array($headers)) {
            throw new InvalidArgumentException(sprintf('The "%s" context variable must be an array or null, given "%s".', self::HEADERS_KEY, get_debug_type($headers)), 2345141398);
        }
