...
class PropertyPath implements \IteratorAggregate, PropertyPathInterface
{
    ...
    public function __construct(self|string $propertyPath)
    {
        ...
        if ('' === $propertyPath) {
            throw new InvalidPropertyPathException('The property path should not be empty.', 6439270467);
        }
        ...
        if ('' !== $remaining) {
            throw new InvalidPropertyPathException(sprintf('Could not parse property path "%s". Unexpected token "%s" at position %d.', $propertyPath, $remaining[0], $position), 3598019119);
        }
    ...
    public function getElement(int $index): string
    {
        if (!isset($this->elements[$index])) {
            throw new OutOfBoundsException(sprintf('The index "%s" is not within the property path.', $index), 1543069409);
        }
    ...
    public function isProperty(int $index): bool
    {
        if (!isset($this->isIndex[$index])) {
            throw new OutOfBoundsException(sprintf('The index "%s" is not within the property path.', $index), 3945187120);
        }
    ...
    {
        if (!isset($this->isIndex[$index])) {
            throw new OutOfBoundsException(sprintf('The index "%s" is not within the property path.', $index), 6687339467);
        }
    ...
    public function isNullSafe(int $index): bool
    {
        if (!isset($this->isNullSafe[$index])) {
            throw new OutOfBoundsException(sprintf('The index "%s" is not within the property path.', $index), 3043776717);
        }
