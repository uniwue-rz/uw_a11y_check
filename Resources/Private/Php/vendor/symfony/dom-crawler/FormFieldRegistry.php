...
class FormFieldRegistry
{
    ...
    public function &get(string $name): FormField|array
    {
        ...
        while ($segments) {
            $path = array_shift($segments);
            if (!\is_array($target) || !\array_key_exists($path, $target)) {
                throw new \InvalidArgumentException(sprintf('Unreachable field "%s".', $path), 1757588846);
            }
            $target = &$target[$path];
        }
    ...
    public function set(string $name, mixed $value): void
    {
        ...
        if ((!\is_array($value) && $target instanceof Field\FormField) || $target instanceof Field\ChoiceFormField) {
            ...
            throw new \InvalidArgumentException(sprintf('Cannot set value on a compound field "%s".', $name), 6673767089);
        }
    }
