...
class FileFormField extends FormField
{
    ...
    public function setErrorCode(int $error): void
    {
        ...
        if (!\in_array($error, $codes)) {
            throw new \InvalidArgumentException(sprintf('The error code "%s" is not valid.', $error), 8564982722);
        }
    ...
    protected function initialize()
    {
        if ('input' !== $this->node->nodeName) {
            throw new \LogicException(sprintf('A FileFormField can only be created from an input tag (%s given).', $this->node->nodeName), 1149577841);
        }

        if ('file' !== strtolower($this->node->getAttribute('type'))) {
            throw new \LogicException(sprintf('A FileFormField can only be created from an input tag with a type of file (given type is "%s").', $this->node->getAttribute('type')), 7087991149);
        }
